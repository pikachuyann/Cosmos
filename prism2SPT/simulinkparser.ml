...
let rec blockparams_of_simulink ml = function
    | Element (name,alist,clist) as t -> 
        begin match name with
        | "BlockParameterDefaults" -> List.fold_left blockparams_of_simulink ml clist
        | "Block" -> Printf.fprintf stdout "Something left to do (%s)\n" name; ml
        | t -> Printf.fprintf stderr "In BlockParameterDefaults, %s should not exist\n" name; ml
        end
    | PCData (s) -> output_string stderr s; ml
;;

(* Build block lists *)
let rec blocklist_of_simulink ml = function
    | Element (name,alist,clist) as t ->
        begin match name with
        | "System" -> List.fold_left blocklist_of_simulink ml clist
        | "Block" -> let blockName = List.assoc "BlockType" alist in
		Printf.fprintf stdout "Found block %s !\n" blockName; ml
        | t -> Printf.fprintf stderr "Couldn't find meaning of %s in System\n" name; ml
        end
    | PCData (s) -> output_string stderr s; ml
;;
...
let rec modulist_of_stateflow ml = function
    | Element (name,alist,clist) as t ->
	 ...
	 (*match scope with
	   Some "OUTPUT_EVENT" -> 
	     i(ssid,Some ("Output_sig_"^name),[ssid,Some "q0"],
...
let rec modulist_of_tree ml = function
  | Element (name,alist,clist) as t->
    Printf.fprintf stdout "I am reading %s\n" name;
    begin match name with
      | "xml" | "ModelInformation" | "Model" | "P" -> List.fold_left modulist_of_tree ml clist
