...
int remaining1(int t){
	...
	int t2 = fmin(ceil(t /((double)(N/N1))),tock);
	tock -= t2;
	//cerr << t2 << endl;
	return t2;
}
...
int remaining2(int t){
	//cerr << "remaining tock:" << tock << " t: " << t << " remain: ";
	int t2 = fmin(ceil(t/((double)(N/N2))),tock);
	tock -= t2;
	//cerr << t2 << endl;
	return t2;
}
...
void SPN::lumpingFun(const abstractMarking &Marking,vector<int> &vect){
  ...
  /*
	...
	vect[reducePL_RE_ERKP] = Marking.P->_PL_ERKP + fmax(0,Marking.P->_PL_ERK_MEKPF-2+vect[reducePL_RE_ERKP_MgiedEKPP]); //To Complete
	...
	*/
	...
	tock = 4*N1 - vect[reducePL_RE_MEK_RafP] - vect[reducePL_RE_MEKP_RafP] ;
	...
	/*
	vect[reducePL_RE_RafP] = Marking.P->_PL_RafP +  vect[reducePL_RE_MEK_RafP] + vect[reducePL_RE_MEKP_RafP] ;
